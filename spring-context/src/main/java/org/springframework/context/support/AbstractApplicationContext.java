...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			 ...
			 * 	2、设置活跃状态为 true
			 * 	3、设置关闭状态为 false
			 * 	4、获取 Environment对象，并加载当前系统的属性值到 Environment 对象中
			 * 	5、获取监听器和事件的集合对象，默认为空的集合
			 */
			prepareRefresh();
			...
			try {
				...
				// 4. 执行后置处理器的相关操作（扩展）,子类覆盖方法做额外的处理，此处一般不做任何扩展工作，可以查看 web 中的代码，有具体的实现
				postProcessBeanFactory(beanFactory);

				// Invoke factory processors registered as beans in the context.
				/**
				 * 5. 开始执行 BeanFactory 后置处理器
				 * 实现化调用所有注册的 BeanFactory 后置处理器
				 * 在单例对象执行之前必须先调用
 				 */
				...
				// 9. 留给子类做扩展工作,初始化其他 bean（SpringBoot 在该方法中启动了 Tomcat 容器）
				onRefresh();
				...
				// 为防止资源占用，在异常处理中，销毁已经在前面过程中生成的单例 bean
				destroyBeans();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 设置几个自动装配的特殊规则，ioc 进行初始化时如果有多个实现，那么就使用指定的对象进行注入
		...
		// 增加对 AspectJ 的支持，在 java 中织入分为三种：分别为编译器织入、 类加载器织入、运行期织入；
		// 1. 编译器织入是指在 java 编译器，采用特殊的编译器，将切面织入到 java 类中
		// 2. 类加载器织入是指通过特殊的类加载器，在类字节码加载到 jvm 时，织入切面，
		// 3. 运行期织入则是采用 cglib 和  jdk 进行切面织入
		// aspectJ 提供了两种织入方式，第一种是通过特殊的编译器，在编译期，将aspectJ语言编写的切面织入到java类中；
		// 第二种是类加载期织入， 即下面的 loadTimeWeaver
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		// 默认情况下，通过 getBeanFactoryPostProcessors() 来获取已经注册的 BeanFactoryPostProcessor，但是默认是空的
	...
	protected void registerListeners() {
		...
		// 从容器中获取所有实现了 ApplicationListener 接口的 BeanDefinition 的 beanName,放入 ApplicationListenerBeans 集合中
	...
	protected void finishRefresh() {
		...
		// LiveBeansView：Spring 用于支持 JMX 服务的类

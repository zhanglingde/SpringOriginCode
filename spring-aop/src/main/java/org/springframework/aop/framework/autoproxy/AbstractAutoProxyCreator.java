...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) {
		...
		if (!StringUtils.hasLength(beanName) || !this.targetSourcedBeans.contains(beanName)) {
			// 查缓存判断是否有处理过（不管是不是需要增强通知的，只要处理过了就会放在里面）
			if (this.advisedBeans.containsKey(cacheKey)) {
				return null;
			}
			if (isInfrastructureClass(beanClass) || shouldSkip(beanClass, beanName)) {
				// 要跳过的直接设置 false（Spring 内置的基本类需要跳过不处理）
	...
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		...
		// 无需增强，advisedBeans 缓存了已经进行了代理的 bean，如果缓存中存在，直接返回
		...
		// 2. 如果存在增强方法则创建代理，获取当前 bean 的 Advices 和 Advisors
		...
		if (specificInterceptors != DO_NOT_PROXY) {
			...
			// 3. 根据获取到的 Advices 和 Advisors 为当前 bean 生成代理对象
